...
resource "metalcloud_infrastructure" "tenant2" {
  
  infrastructure_label = "tenant2-2"
  ...
  shared_drive {
    ...
    shared_drive_size_mbytes = 400965
    shared_drive_storage_type = "iscsi_hdd"
    shared_drive_attached_instance_arrays = ["esxi-group1-1"]
  }
  ...
  instance_array {
    # Name of your cluster. Needs to obey DNS rules as it will translate into a DNS record.
    instance_array_label = "esxi-group2-1"
    ...
    instance_array_additional_wan_ipv4_json = jsonencode(
                {
                   configs = [
                       ...
                       {
                           forced_subnet_pool_id = 11
                           override_vlan_id      = 400
                        },
                       {
                           forced_subnet_pool_id = 12
                           override_vlan_id      = 500
                        },
                    ]
                }
            )
  ...
  shared_drive {
    shared_drive_label = "my-shared-drive-2"
    shared_drive_size_mbytes = 400965
    shared_drive_storage_type = "iscsi_hdd"
    shared_drive_attached_instance_arrays = ["esxi-group2-1"]
  }

  shared_drive {
    shared_drive_label = "my-shared-drive-3"
    shared_drive_size_mbytes = 400965
    shared_drive_storage_type = "iscsi_hdd"
    shared_drive_attached_instance_arrays = ["esxi-group2-1"]
  }
